 ...
 * - Correctly passes the 'currentUser.uid' to the apiClient, fixing the
 * critical backend authentication issue.
 */
...
// SVG Paths are preserved for UI consistency
...
const MainMenuCrossAnalyzer = ({ onNavigateToDashboard }) => {
    ...
    const handleAnalyzeFileClick = () => {
        if (!currentUser) {
            showAppStatusMessage('Proszę się zalogować, aby rozpocząć analizę.', 'error');
            return;
        }
        if (!selectedFile) {
    ...
    const processAnalysisWithName = async (analysisName) => {
        ...
        if (!selectedFile || !currentUser) {
            showAppStatusMessage('Błąd: Plik lub użytkownik nie jest już dostępny.', 'error');
            return;
        }
        ...
        try {
            ...
            const storagePath = `raw_csvs/${currentUser.uid}/${analysisId}/${selectedFile.name}`;
            const storageRef = ref(storage, storagePath);
            await uploadBytes(storageRef, selectedFile);

            showAppStatusMessage(`Krok 2/2: Inicjowanie analizy "${analysisName}"...`, 'info', null);
            
            // CRITICAL FIX: Pass the currentUser.uid to the backend.
            const result = await apiClient.requestAnalysis(analysisId, selectedFile.name, analysisName, currentUser.uid);
    ...
    const handleBrowseAnalyses = () => {
         if (!currentUser) {
            showAppStatusMessage('Proszę się zalogować, aby przeglądać analizy.', 'error');
            return;
        }
        onNavigateToDashboard({});
    };
    ...
    const isDisabled = !currentUser || isLoadingAnalyze || isLoadingBrowse;
    ...
    return (
        <div className="new-landing-page-body-wrapper">
            <div className="analyzer-card">
                ...
                <NewStyledButton id="analyzeFileBtn" label="Analizuj Plik" variant="primary" iconSvgPath={AnalyzeIconPath} onClick={handleAnalyzeFileClick} disabled={!selectedFile || isDisabled} isLoading={isLoadingAnalyze} loadingText="Przetwarzam..." className={analyzeButtonCueClass} />
                ...
                <NewStyledButton id="browseAnalysesBtn" label="Przeglądaj Analizy" variant="secondary" iconSvgPath={BrowseIconPath} onClick={handleBrowseAnalyses} disabled={isDisabled} isLoading={isLoadingBrowse} loadingText="Ładowanie..." />
